...
export class ScreenMapper {
    ...
    drawGrids() {
        //get a copy of the graphics obj

        const graphicContext = this.config.scene.add.graphics()
        //set pen characteristics
        graphicContext.lineStyle(2, 0xff0000, 0.5)
        for (let i = 0; i < this.config.columns; i++) {
            //move the pen to the first location 
            //drawing columns
            graphicContext.moveTo(i * this.gridWidth, 0)
            graphicContext.lineTo(i * this.gridWidth, this.scrHeight)

        }
        //drawing rows
        for (let i = 0; i < this.config.rows; i++) {
            graphicContext.moveTo(0, i * this.gridHeight)
            graphicContext.lineTo(this.scrWidth, i * this.gridHeight)
        }
